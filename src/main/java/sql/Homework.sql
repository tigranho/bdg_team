...
/*3*/
SELECT maker, COUNT(maker) FROM (SELECT DISTINCT maker, type FROM Product) AS A
GROUP BY maker;

/*4*/
SELECT AVG(price) AS AVERAGE_PRICE FROM Laptop;
...
/*7*/
SELECT SUM(price) FROM (SELECT maker, code, Product.model, speed, price FROM Product
LEFT JOIN PC
ON PC.model = Product.model) AS A;
...
/*9*/
SELECT maker FROM (SELECT DISTINCT maker, Product.type FROM Product
LEFT JOIN Printer
ON Product.model = Printer.model
WHERE Product.type = 'PC' OR (Product.type = 'Printer' AND color = 'y')) AS A
GROUP BY maker
