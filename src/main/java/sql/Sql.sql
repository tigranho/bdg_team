...
ALTER TABLE jdbc.Laptop  ADD
	CONSTRAINT PK_Laptop PRIMARY KEY  NONCLUSTERED
	(
		code
	) ;

ALTER TABLE jdbc.PC  ADD
...
SELECT  maker,COUNT(model) AS EMP FROM product group by maker;
use taburetka;
select maker,count(distinct type) from product group by maker;
select  AVG(price) as emp from laptop;

SELECT pc.CODE, pc.MODEL, pc.SPEED, pc.PRICE , PRODUCT.MAKER
FROM pc  INNER JOIN PRODUCT ON PC.MODEL=PRODUCT.MODEL;

SELECT PRINTER.CODE, PRINTER.MODEL ,PRINTER.PRICE, PRODUCT.MAKER
FROM PRINTER INNER JOIN PRODUCT ON PRINTER.MODEL = PRODUCT.MODEL order by  printer.price desc;

SELECT product.MAKER, pc.CODE, pc.MODEL, pc.SPEED, pc.PRICE
FROM PRODUCT LEFT  JOIN PC ON  PRODUCT.MODEL=PC.MODEL;

SELECT product.MAKER, ifnull(avg(pc.price),0) AS avgprice
FROM PRODUCT LEFT  JOIN PC ON  PRODUCT.MODEL=PC.MODEL group by maker;


SELECT DISTINCT pcmaker.maker FROM
(SELECT maker
FROM (PRODUCT JOIN PC ON  PRODUCT.MODEL=PC.MODEL )  )  AS pcmaker
JOIN
(SELECT maker
FROM (PRODUCT JOIN PRINTER ON  PRODUCT.MODEL=PRINTER.MODEL and  printer.color = 'Y') ) AS printermaker
...
SELECT  maker
FROM PRODUCT
WHERE PRODUCT.type ='PRINTER')) AS notprinter join laptop on notprinter.model = laptop.model and laptop.speed >=500;