...
import jdbctaskusingjpaproviderhibernate.dao.CompanyDao;
...
public class CompanyDaoImpl implements CompanyDao {
    ...
    public Company update(Company company) throws SQLException {
        ...
        Query query = entityManager.createQuery(
                "UPDATE Company as p SET p.name=:n,  p.foundDate=:p1 WHERE p.comapnyId=:id");
        int updateCount = query.setParameter("n", company.getName())
                .setParameter("p1", company.getFoundDate())
                .setParameter("id", company.getComapnyId()).executeUpdate();
        entityManager.getTransaction().commit();
        entityManager.close();
        e.close();
        return company;
    ...
    public void delete(long companyId) throws SQLException {
        ...
        entityManager.getTransaction().begin();
        Query query = entityManager.createQuery(
                "delete from Company as p WHERE p.comapnyId=:id");
